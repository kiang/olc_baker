 ...
 */
    function manage($id = null) {
        ...
        if (!empty($this->data)) {
            $cascade = (isset($this->data['PermissibleAco']['cascade']) && $this->data['PermissibleAco']['cascade'] === '1');
            unset($this->data['PermissibleAco']['cascade']);
            foreach ($this->data['PermissibleAco'] as $aro_alias => $perm) {
                $perm = ((int)$perm === 1);
                if ($perm) {
                    if ($cascade) {
